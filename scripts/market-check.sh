...
#    https://a2hmarket.ai/skill/codex/a2hmarket-skill.zip 由发包流水线按 profile 注入；仓库工作树里是占位符 → 整段自动跳过（本地开发不联网查）。
try:
    import sys as _sys, time as _time, urllib.request as _rq
    if STATE is None:
        raise RuntimeError("无状态巡查：频控与「每版只提一次」都要落状态，没状态目录就整段跳过")
    _dl = "https://a2hmarket.ai/skill/codex/a2hmarket-skill.zip"
